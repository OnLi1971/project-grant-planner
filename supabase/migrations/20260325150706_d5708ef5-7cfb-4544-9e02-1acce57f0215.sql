-- Resolve RPC overload ambiguity for engineers_create / engineers_update.
-- Keep a single canonical overload with the full argument set used by the app.

DROP FUNCTION IF EXISTS public.engineers_update(uuid, text, text, public.engineer_status, integer, uuid, uuid);
DROP FUNCTION IF EXISTS public.engineers_update(uuid, text, text, public.engineer_status, integer, uuid, uuid, text, numeric, text);
DROP FUNCTION IF EXISTS public.engineers_update(uuid, text, text, public.engineer_status, integer, uuid, uuid, text, numeric, text, text);

CREATE OR REPLACE FUNCTION public.engineers_update(
  p_id uuid,
  p_display_name text DEFAULT NULL::text,
  p_email text DEFAULT NULL::text,
  p_status public.engineer_status DEFAULT NULL::public.engineer_status,
  ...
  p_software text DEFAULT NULL::text,
  p_pdm_plm text DEFAULT NULL::text,
  p_specialization text DEFAULT NULL::text
)
RETURNS public.engineers
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  v_row public.engineers;
  v_slug text;
BEGIN
  SELECT * INTO v_row FROM public.engineers WHERE id = p_id FOR UPDATE;
  IF NOT FOUND THEN
  ...
  IF p_display_name IS NOT NULL THEN
    v_slug := public.normalize_slug(p_display_name);
    IF EXISTS (SELECT 1 FROM public.engineers WHERE slug = v_slug AND id <> p_id) THEN
...
DROP FUNCTION IF EXISTS public.engineers_create(text, text, public.engineer_status, integer, uuid, uuid);
DROP FUNCTION IF EXISTS public.engineers_create(text, text, public.engineer_status, integer, uuid, uuid, text, numeric, text);
DROP FUNCTION IF EXISTS public.engineers_create(text, text, public.engineer_status, integer, uuid, uuid, text, numeric, text, text);

CREATE OR REPLACE FUNCTION public.engineers_create(
  p_display_name text,
  p_email text DEFAULT NULL::text,
  p_status public.engineer_status DEFAULT 'active'::public.engineer_status,
  p_fte integer DEFAULT 100,
  p_department uuid DEFAULT NULL::uuid,
  p_manager uuid DEFAULT NULL::uuid,
  p_company text DEFAULT 'TM CZ'::text,
  p_hourly_rate numeric DEFAULT NULL::numeric,
  p_currency text DEFAULT NULL::text,
  p_location text DEFAULT 'PRG'::text,
  p_software text DEFAULT NULL::text,
  p_pdm_plm text DEFAULT NULL::text,
  p_specialization text DEFAULT NULL::text
)
...
BEGIN
  ...
  PERFORM 1 FROM public.engineers WHERE slug = v_slug;
  IF FOUND THEN
