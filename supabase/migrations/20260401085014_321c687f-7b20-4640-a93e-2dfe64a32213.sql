...
DROP FUNCTION IF EXISTS public.engineers_create(text, text, engineer_status, integer, uuid, uuid, text, numeric, text, text, text, text, text);
DROP FUNCTION IF EXISTS public.engineers_update(uuid, text, text, engineer_status, integer, uuid, uuid, text, numeric, text, text, text, text, text);

-- Recreate engineers_create with p_end_date
...
BEGIN
  ...
  INSERT INTO public.engineers (
    display_name, slug, email, status, fte_percent,
    department_id, manager_id, company, hourly_rate, currency,
    location, software, pdm_plm, specialization, end_date
  )
  VALUES (
    p_display_name, v_slug, p_email, p_status, COALESCE(p_fte, 100),
    p_department, p_manager, p_company, p_hourly_rate, p_currency,
    COALESCE(p_location, 'PRG'), p_software, p_pdm_plm, p_specialization, p_end_date
  )
...
-- Recreate engineers_update with p_end_date
...
BEGIN
  ...
  IF p_end_date IS NOT NULL THEN v_row.end_date := p_end_date; END IF;
