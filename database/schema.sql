...
-- INSERT INTO public.profiles (id, email, full_name, role)
-- VALUES ('YOUR_USER_ID_HERE', 'your-email@example.com', 'Admin User', 'admin')
...
-- SELECT p.full_name, p.email, p.role, p.created_at 
-- FROM public.profiles p ORDER BY p.created_at;

-- View planning data with user info
-- SELECT pe.*, p.full_name as updated_by_name
-- FROM public.planning_entries pe
-- LEFT JOIN public.profiles p ON pe.updated_by = p.id
-- ORDER BY pe.konstrukter, pe.cw;